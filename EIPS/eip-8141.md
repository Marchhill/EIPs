...
**Gas.** The validation path is dispatch, three `SIGPARAM` reads, `APPROVE` — whose only metered cost is memory expansion, and thus zero for the empty return region — and one cold `SLOAD` of slot 0. Everything but the `SLOAD` is a small fixed overhead, so the cost is essentially a single cold storage read on the active fork's schedule, comfortably below the 15,000 `pay`-frame `gas_limit` on any current schedule. Nodes MAY reject canonical `pay` frames with a `gas_limit` above that bound as non-canonical usage.
