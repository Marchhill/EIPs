...
Its per-fork `keccak256` code hash is `0xda42f0d11838c4c0c3129b8b8e93e9718127ad6b315e517e1088125707c4d45c`. An annotated assembly listing that assembles byte-for-byte to this bytecode, together with the bytecode and hash, is given in [`assets/eip-8141/canonical-paymaster.md`](../assets/eip-8141/canonical-paymaster.md); all three are reproducible from the reference assembler.

**Gas.** The validation path costs a single cold `SLOAD` of slot 0 plus dispatch, three `SIGPARAM` reads, and `APPROVE` — roughly 3,100 gas at the state-access prices of this fork (dominated by the cold `SLOAD`; the reference implementation measures 3,110). A canonical `pay` frame's `gas_limit` of 15,000 is sufficient with wide margin, and nodes MAY reject canonical `pay` frames with a `gas_limit` above that bound as non-canonical usage.
