...
**Gas.** The validation path is dominated by a single cold `SLOAD` of slot 0, plus a small fixed overhead, so it stays well below the 15,000 `pay`-frame `gas_limit` on any current schedule. Nodes MAY reject canonical `pay` frames with a `gas_limit` above that bound as non-canonical usage.
